...
struct ExportEditedImageHelper {
    ...
    static func exportEditedImage(from view: some View, onComplete: @escaping (Bool, String) -> Void) {
        ...
        guard let editorImage = renderer.uiImage else {
            onComplete(false, "Cannot create image.")
            return
        }
        ...
        func save(_ image: UIImage) {
            PHPhotoLibrary.shared().performChanges {
                ...
                DispatchQueue.main.async {
                    if let error = error {
                        onComplete(false, "Error save: \(error.localizedDescription)")
                    } else {
                        onComplete(true, "Saved successfully.")
                    }
                }
            }
        }
        ...
        case .notDetermined:
            PHPhotoLibrary.requestAuthorization(for: .addOnly) { newStatus in
                if newStatus == .authorized || newStatus == .limited {
                    ...
                    DispatchQueue.main.async {
                        onComplete(false, "You have denied permission to save the image.")
                    }
                }
            }
        default:
            onComplete(false, "No permission to save image. Please check Settings.")
        }
    }
}
